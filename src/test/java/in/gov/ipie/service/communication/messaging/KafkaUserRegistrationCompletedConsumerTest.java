...
class KafkaUserRegistrationCompletedConsumerTest {

    private final NotificationService notificationService = mock(NotificationService.class);
    private final ProcessedEventStore processedEventStore = mock(ProcessedEventStore.class);
    private final ObjectMapper objectMapper = new ObjectMapper();
    private final KafkaUserRegistrationCompletedConsumer consumer =
            new KafkaUserRegistrationCompletedConsumer(notificationService, processedEventStore, objectMapper);

    @Test
    void onEvent_sendsBothNotifications_forAMatchingEventType() {
        UserRegistrationCompletedPayload payload = new UserRegistrationCompletedPayload(
                UUID.randomUUID(), "jane@example.com", "Jane Doe", "+91 9800000009", "tok-123", Set.of(NotificationChannel.EMAIL));
        EventEnvelope<UserRegistrationCompletedPayload> event =
