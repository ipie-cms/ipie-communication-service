package in.gov.ipie.service.communication.architecture;
 ...
 * provides (master standards doc binding rules, Section 13). A new service created from
 * {@code ipie-service-template} keeps this test unchanged except for the base package.
 *
 * <p>This service was missing this file entirely until now, while
 * {@code ipie-user-service}, {@code ipie-iam-service} and {@code ipie-service-template} each
 * carried one - so none of these rules were enforced here. That gap is how a business condition in
 * {@code NotificationServiceImpl} came to be signalled with a raw {@code IllegalStateException}
 * instead of an {@link in.gov.ipie.common.core.exception.IpieException} subtype.
 */
@AnalyzeClasses(packages = "in.gov.ipie.service.communication")
class ArchitectureTest {

    private static final String BASE_PACKAGE = "in.gov.ipie.service.communication";
