...
class KafkaUserLoggedInConsumerTest {

    private final NotificationService notificationService = mock(NotificationService.class);
    private final ProcessedEventStore processedEventStore = mock(ProcessedEventStore.class);
    private final ObjectMapper objectMapper = new ObjectMapper().registerModule(new JavaTimeModule());
    private final KafkaUserLoggedInConsumer consumer =
            new KafkaUserLoggedInConsumer(notificationService, processedEventStore, objectMapper);

    @Test
    void onEvent_sendsTheLoginNotification_forAMatchingEventType() {
        UserLoggedInPayload payload = new UserLoggedInPayload(
        ...
        EventEnvelope<UserLoggedInPayload> event = EventEnvelope.create("USER_LOGGED_IN", 1, "ipie-user-service", null, null, payload);
