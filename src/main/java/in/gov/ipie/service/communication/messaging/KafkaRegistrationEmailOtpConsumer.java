...
/**
 * Kafka counterpart to {@link RegistrationEmailOtpConsumer} - active only when this service
 * is configured to use Kafka instead of RabbitMQ. Same filter-the-whole-topic shape as {@link
 * KafkaUserRegistrationCompletedConsumer} - see that class's Javadoc.
 */
@Component
@ConditionalOnProperty(prefix = "spring.kafka", name = "bootstrap-servers")
class KafkaRegistrationEmailOtpConsumer {

    private static final String EVENT_TYPE = "REGISTRATION_EMAIL_OTP_REQUESTED";

    private static final Logger LOG = LoggerFactory.getLogger(KafkaRegistrationEmailOtpConsumer.class);

    private final NotificationService notificationService;
    private final ProcessedEventStore processedEventStore;
    private final ObjectMapper objectMapper;

    KafkaRegistrationEmailOtpConsumer(
    ...
    void onEvent(EventEnvelope<?> event) {
        ...
        IdempotentEventHandler.handle(event.eventId(), processedEventStore, () -> {
            RegistrationEmailOtpPayload payload = objectMapper.convertValue(event.data(), RegistrationEmailOtpPayload.class);
