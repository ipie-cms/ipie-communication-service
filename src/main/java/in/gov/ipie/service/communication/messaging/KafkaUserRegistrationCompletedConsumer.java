...
/**
 * Kafka counterpart to {@link UserRegistrationCompletedConsumer} - active only when this
...
class KafkaUserRegistrationCompletedConsumer {

    private static final String EVENT_TYPE = "USER_REGISTRATION_COMPLETED";

    private static final Logger LOG = LoggerFactory.getLogger(KafkaUserRegistrationCompletedConsumer.class);

    private final NotificationService notificationService;
    private final ProcessedEventStore processedEventStore;
    private final ObjectMapper objectMapper;

    KafkaUserRegistrationCompletedConsumer(
    ...
    void onEvent(EventEnvelope<?> event) {
        ...
        IdempotentEventHandler.handle(event.eventId(), processedEventStore, () -> {
            UserRegistrationCompletedPayload payload =
                    objectMapper.convertValue(event.data(), UserRegistrationCompletedPayload.class);
